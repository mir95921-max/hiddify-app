                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"VextraVPN"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"HiddifyMutex");
  HWND handle = FindWindowA(NULL, "VextraVPN");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"VextraVPN"))
    {
      return false;
    }
  ...
  if (!window.Create(L"VextraVPN", origin, size))
  {
    return EXIT_FAILURE;
  }
